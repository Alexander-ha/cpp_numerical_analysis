...
array< double, 2 > &jacstep( array< double, 2 > &ksol,  array< double, 2 > &start){
    //подсчет переменных якобиана
    double x00 = 6*pow(start[0], 5);
    double x01 = 6*pow(start[1], 5);
    double x10 = -exp(start[0]);
    double x11 = exp(start[1]);
...
int main(){
    int eps = 0.001;
    int iters = 0;
    std::array<double, 2> ksol = {5, 6.7};
    std::array<double, 2> start = ksol;
//    std::array<double, 2> osol = {1, 1};
//    cout<<euclid_norm(osol)<<endl;
//    std::array<double, 2> sisol = substract(osol, ksol);
    std::array<double, 2> ssol = substract(ksol, jacstep(ksol, start));

    ksol = ssol; 

    ssol = substract(ksol, jacstep(ksol, start));
