...
double exact_sol(double tmp, int a){
    return exp(-a*tmp);
}

//подсчет коэффициентов и численного решения для step+1
void Runge_Kutta(int step, double tmp, double h, double exact_sol(double, int), Solarr num_resh, double* y1, double* y2, double y_hp1, double y_hp2, double matrix[4][4]){
    double k1[3];
    double k2[3];
...
double max_norm(double exact_sol(double, int), Solarr num_resh, double tmp, double y1, double y2){
    Solarr recount = func_resh(tmp, y1, y2);
    double* y0 = recount.num_sol; 
    double norm0 = abs(sqrt((exact_sol(tmp,4)*exact_sol(tmp,4) - y0[0]*y0[0])*(exact_sol(tmp,4)*exact_sol(tmp,4) - y0[0]*y0[0]) + (exact_sol(tmp,1)*exact_sol(tmp,1)-y0[1]*y0[1])*(exact_sol(tmp,1)*exact_sol(tmp,1)-y0[1]*y0[1])));
...
int main(){
    ...
    double eps = 10e-10;
    Solarr num_resh;
    double t_start = 0, y1 = 0, y2 = 0, t_end = 4, h = 0.01;
